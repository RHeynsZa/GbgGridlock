...
async def fetch_line_metadata_once(pool, vt_client: VasttrafikClient, target_stop_area_gids: list[str], http_concurrency: int) -> None:
...
async def run_poll_cycle(pool, vt_client: VasttrafikClient, target_stop_area_gids: list[str], http_concurrency: int) -> None:
    semaphore = asyncio.Semaphore(http_concurrency)
