...
export function DashboardPage() {
  ...
  const selectedDistributionLine = selectedLine || worstLinesQuery.data?.[0]?.line_number || '11'
  const distributionBarColor = getLineStyle(selectedDistributionLine).backgroundColor
          ...
          <div className="lg:col-span-9">
            <Card className="rounded-xl shadow-md">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={hourlyTrendQuery.data ?? []} margin={{ top: 10, right: 10, left: 0, bottom: 24 }}>
                  <defs>
                    ...
                    <div key={line.line} className="group cursor-pointer">
                      <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2 rounded-lg p-2 transition-colors hover:bg-muted/50 sm:gap-3">
                        ...
                        <div className="relative h-8 overflow-hidden rounded-lg border border-border/60 bg-muted/70 sm:h-10">
                          <div
                            className="flex h-full items-center justify-end rounded-lg border-b-4 border-r-4 px-1.5 transition-all duration-300 group-hover:brightness-105 group-hover:shadow-sm sm:px-2"
                            style={{
                              width: `${widthPercent}%`,
                              borderBottomColor: getLineStyle(line.line).backgroundColor,
                              borderRightColor: getLineStyle(line.line).backgroundColor,
                              backgroundColor: withAlpha(getLineStyle(line.line).backgroundColor, 'CC'),
                              color: getLineStyle(line.line).textColor,
                            }}
                          >
                            <span className="text-[10px] font-bold sm:text-xs">{line.avgDelaySeconds}s</span>
                          </div>
                        </div>
                      ...
                      style={
                        selectedLine === line.line
                          ? {
                              borderBottomColor: getLineStyle(line.line).backgroundColor,
                              borderRightColor: getLineStyle(line.line).backgroundColor,
                              backgroundColor: withAlpha(getLineStyle(line.line).backgroundColor, '1A'),
                            }
                          : {}
                      }
          ...
          <CardContent>
            <div className="h-[320px] w-full sm:h-[360px]">
              {delayDistributionData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={delayDistributionData} barCategoryGap="20%" margin={{ top: 10, right: 10, left: 0, bottom: 24 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} strokeOpacity={0.5} />
                    <XAxis 
                      dataKey="delayRange" 
                      stroke="var(--muted-foreground)"
