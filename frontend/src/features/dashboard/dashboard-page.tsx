...
const fallbackLineStyles: Record<
  string,
...
function mapTransportModeToLineMode(
...
export function DashboardPage() {
  ...
  const worstLinesQuery = useQuery({
    ...
    queryFn: () =>
      fetchWorstLines(
        selectedStop === "all" ? undefined : selectedStop,
        timeRange.minutes,
      ),
  });
  ...
  const cancellationRate = useMemo(() => {
    return Number(
      (networkStatsQuery.data?.cancellation_rate_percent ?? 0).toFixed(1),
    );
  }, [networkStatsQuery.data]);
  ...
  const filteredLines = useMemo(
    () =>
      lineDrilldown.filter((line) =>
        selectedMode === "All" ? true : line.mode === selectedMode,
      ),
  ...
  const selectedLineStats = useMemo(
    () =>
      filteredLines.find((line) => line.line === selectedLine) ??
      filteredLines[0] ??
      null,
  ...
  const lineDelayRanking = useMemo(() => {
    ...
    return [...lineDrilldown].sort(
      (a, b) => b.avgDelaySeconds - a.avgDelaySeconds,
    );
  ...
  const distributionBarColor = selectedLineStats
    ? getLineStyle(selectedLineStats.line).backgroundColor
    : getLineStyle(
        selectedLine || worstLinesQuery.data?.[0]?.line_number || "11",
      ).backgroundColor;
  ...
  const formatKpi = (value: number, suffix: string) =>
    `${new Intl.NumberFormat(i18n.language).format(value)}${suffix}`;
  const translateMode = (mode: LineMode | "All") =>
    t(`dashboard.modes.${mode.toLowerCase()}`);
    ...
    <main className="min-h-screen w-full">
      ...
      <div className="mx-auto w-full max-w-7xl space-y-4 px-3 py-4 sm:space-y-6 sm:px-6 sm:py-6 lg:space-y-8 lg:px-8 lg:py-8">
              ...
              <Card key={kpi.title} className="kpi-card rounded-xl">
                <CardHeader className="pb-2 sm:pb-3">
                  <div className="flex items-start justify-between">
                    ...
                    <div
                      className={`rounded-lg bg-muted p-1.5 sm:p-2 ${kpi.color}`}
                    >
                      {kpi.icon}
                    </div>
                  </div>
                  <CardTitle className="mt-1 text-2xl font-bold sm:mt-2 sm:text-3xl md:text-4xl">
                    {kpi.value}
                  </CardTitle>
                </CardHeader>
                  ...
                  <select
                    ...
                    onChange={(event) => {
                      const selected = TIME_RANGES.find(
                        (tr) => tr.label === event.target.value,
                      );
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        tickFormatter={(value) => {
                          ...
                          if (parts.length === 2) {
                            ...
                            const uniqueDates = new Set(
                              hourlyData.map((d) => d.hour.split(" ")[0]),
                            );
                            return uniqueDates.size > 1
                              ? `${date.substring(5)} ${time.substring(0, 5)}`
                              : time.substring(0, 5);
                          }
                          return value;
                        }}
                      ...
                      <Area
                        type="monotone"
                        dataKey="ferry"
                        name={translateMode("Ferry")}
                        stroke="var(--chart-3)"
                        fill="url(#ferryGradient)"
                        strokeWidth={2.5}
                      />
                      <Legend
        ...
        <Card className="rounded-xl shadow-md">
          <CardHeader className="pb-3 sm:pb-6">
            <CardTitle className="text-sm sm:text-base">
              {t("charts.rankingTitle")}
            </CardTitle>
            <CardDescription className="text-xs">
              {t("charts.rankingDesc")}
            </CardDescription>
          </CardHeader>
              ...
              <>
                {lineDelayRanking.map((line, index) => {
                  const widthPercent = Math.max(
                    10,
                    Math.round((line.avgDelaySeconds / maxLineDelay) * 100),
                  );
                  return (
                    <div key={line.line} className="group cursor-pointer">
                      <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2 rounded-lg p-2 transition-colors hover:bg-muted/50 sm:gap-3">
                        ...
                        <div className="relative h-8 overflow-hidden rounded-lg border border-border/60 bg-muted/70 sm:h-10">
                          <div
                            className="flex h-full items-center justify-end rounded-md border-b-4 border-r-4 px-1.5 transition-all duration-300 group-hover:brightness-105 group-hover:shadow-sm sm:px-2"
                            style={{
                              width: `${widthPercent}%`,
                              borderBottomColor: getLineStyle(line.line)
                                .backgroundColor,
                              borderRightColor: getLineStyle(line.line)
                                .backgroundColor,
                              backgroundColor: withAlpha(
                                getLineStyle(line.line).backgroundColor,
                                "CC",
                              ),
                          ...
                          >
                            <span className="text-[10px] font-bold sm:text-xs">
                              {line.avgDelaySeconds}s
                            </span>
                          </div>
                        </div>
                        <p className="text-right text-xs font-bold tabular-nums sm:text-sm">
                          {line.avgDelaySeconds}s
                        </p>
                      </div>
                    </div>
          ...
          <CardContent className="space-y-3 sm:space-y-5">
            {filteredLines.length > 0 ? (
              <>
                <div className="flex flex-wrap gap-2">
                  {filteredLines.map((line) => (
                    <button
                      key={line.line}
                      type="button"
                      className={`chip flex items-center gap-1.5 ${selectedLine === line.line ? "border-b-4 border-r-4" : ""}`}
                      style={
                ...
                {selectedLineStats ? (
                  <div
                    className="rounded-xl border-b-4 border-r-4 border-border bg-muted/30 p-3 sm:p-5"
                    style={{
                      borderBottomColor: getLineStyle(selectedLineStats.line)
                        .backgroundColor,
                      borderRightColor: getLineStyle(selectedLineStats.line)
                        .backgroundColor,
                    }}
                  >
                    <div className="mb-3 flex items-center gap-2 sm:mb-4">
                      <div
                        className="flex h-8 w-8 items-center justify-center rounded-lg border-2 bg-card text-sm font-bold text-foreground sm:h-10 sm:w-10"
                        style={{
                          borderColor: getLineStyle(selectedLineStats.line)
                            .backgroundColor,
                        }}
                      >
                      ...
                      <div>
                        <p className="text-xs font-semibold sm:text-sm">
                          {translateMode(selectedLineStats.mode)} Line{" "}
                          {selectedLineStats.line}
                        </p>
                        <p className="text-[10px] text-muted-foreground sm:text-xs">
                          Detailed statistics
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-3">
                      <div className="space-y-0.5 sm:space-y-1">
                        <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground sm:text-xs">
                          {t("drilldown.avgDelay")}
                        </p>
                        <p className="text-xl font-bold sm:text-2xl">
                          {selectedLineStats.avgDelaySeconds}s
                        </p>
                      </div>
                      <div className="space-y-0.5 sm:space-y-1">
                        <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground sm:text-xs">
                        ...
                        <p className="text-xl font-bold text-success sm:text-2xl">
                          {selectedLineStats.onTimeRate}%
                        </p>
                      </div>
                      <div className="space-y-0.5 sm:space-y-1">
                        <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground sm:text-xs">
                          {t("drilldown.cancellations")}
                        </p>
                        <p className="text-xl font-bold text-error sm:text-2xl">
                          {selectedLineStats.canceledTrips}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex h-32 items-center justify-center text-muted-foreground">
                <p className="text-sm">
                  No line data available for selected filters
                </p>
              </div>
            )}
          </CardContent>
              ...
              {delayDistributionData.length > 0 ? (
                ...
                <div className="flex h-full items-center justify-center rounded-lg border border-dashed border-border">
                  <p className="text-sm text-muted-foreground">
                    {delayDistributionQuery.isLoading
        ...
        <Card className="rounded-xl border-dashed shadow-md">
          <CardHeader className="pb-3 sm:pb-6">
            ...
            <CardDescription className="text-xs">
              Live monitoring metrics for polling and API reliability
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xl font-bold">
                    {debugMetricsQuery.data.monitored_stops_count}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                  ...
                  <p className="text-xl font-bold">
                    {debugMetricsQuery.data.poll_requests_count_5m}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                  ...
                  <p className="text-xl font-bold">
                    {Math.round(
                      debugMetricsQuery.data.average_api_response_time_ms_5m,
                    )}
                    ms
                  </p>
                </div>
                <div className="space-y-1">
                  ...
                  <p className="text-xl font-bold text-success">
                    {debugMetricsQuery.data.success_rate_percent_5m.toFixed(1)}%
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                  ...
                  <p className="text-xl font-bold">
                    {debugMetricsQuery.data.poll_cycles_count_5m}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                  ...
                  <p className="text-xl font-bold text-success">
                    {debugMetricsQuery.data.successful_stop_polls_count_5m}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                  ...
                  <p className="text-xl font-bold text-error">
                    {debugMetricsQuery.data.failed_stop_polls_count_5m}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                  ...
                  <p className="text-xl font-bold">
                    {debugMetricsQuery.data.window_minutes}min
                  </p>
                </div>
              </div>
            ) : (
