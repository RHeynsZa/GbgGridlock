...
test.describe('API integration – mocked backend responses', () => {
  ...
  test('renders chart legends and selection accents for the updated charts', async ({ page }) => {
    await mockAllApiRoutes(page)
    ...
    await expect(page.locator('.recharts-legend-item-text', { hasText: 'Tram' })).toBeVisible()
    await expect(page.locator('.recharts-legend-item-text', { hasText: 'Departures' })).toBeVisible()
